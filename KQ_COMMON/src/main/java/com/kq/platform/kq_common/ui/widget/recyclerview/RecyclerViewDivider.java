...
/**
 * Created by X on 2016/12/6.
 * RecyclerView的自定义分割线类，需要自己绘制。
 * 在此之上添加3种设定方法，分别控制对应的高度，颜色，drawable
 */
public class RecyclerViewDivider extends RecyclerView.ItemDecoration{
    ...
    //是否显示最后的Divider
    private boolean mIsShowEndDivider;
    //系统自带的参数
     ...
     * @param orientation 列表方向
     */
    public RecyclerViewDivider(Context context, int orientation, boolean mIsShowEndDivider) {
        this.setOrientation(orientation);
        //获取xml配置的参数
        final TypedArray a = context.obtainStyledAttributes(ATTRS);
        //typedArray.getDrawable(attr)这句是说我们可以通过我们的资源获得资源，使用我们的资源名attr去获得资源id
        //看不懂就用自己写一个分割线的图片吧，方法：ContextCompat.getDrawable(context, drawableId);
        mDrawable = a.getDrawable(0);
        this.mIsShowEndDivider = mIsShowEndDivider;
        //官方的解释是：回收TypedArray，以便后面重用。在调用这个函数后，你就不能再使用这个TypedArray。
        //在TypedArray后调用recycle主要是为了缓存。
        a.recycle();
    }
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        switch (mOrientation) {
            ...
            case VERTICAL_LIST:
                ...
                outRect.set(0, 0, (itemPosition != childCount || mIsShowEndDivider) ? mDividerHeight : 0, 0);
                break;
            case HORIZONTAL_LIST:
                childCount -= 1;
                //垂直布局底部留边，最后一行不留
                outRect.set(0, 0, 0, (itemPosition != childCount || mIsShowEndDivider) ? mDividerHeight : 0);
                break;
        }
    }
